...
def update_user_account_status(
    ...
    if status_data.action == "suspend":
        ...
        user.is_active = True
        for pending_appeal in database.scalars(
            select(AccountAppeal).where(
                AccountAppeal.user_id == user.id,
                AccountAppeal.status == "pending",
            )
        ):
            pending_appeal.status = "restored"
            pending_appeal.decision_reason = "Account restored by an administrator."
            pending_appeal.reviewer_id = current_admin.id
    ...
    if action == "USER_SUSPENDED":
        send_account_email_safely(
            user.email, "GraduateLink SA: account suspended",
            f"Your GraduateLink SA account has been suspended.\n"
            f"Reason: {status_data.reason.strip()}\n\n"
            f"An administrator can review your account. To appeal, visit "
            f"{settings.frontend_url.rstrip('/')}/appeal or contact "
            f"{settings.support_email or 'GraduateLink SA support'}.\n"
            "Your account will only be restored after a review.\n",
        )
    else:
        send_account_email_safely(
            user.email, "GraduateLink SA: account restored",
            "Your GraduateLink SA account has been restored. You can sign in again.\n",
        )
    return user
