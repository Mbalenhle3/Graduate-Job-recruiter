...
export default function HomePage() {
  return <div className="public-page role-home">
    <header className="public-header"><div className="public-nav"><Logo/><nav><a href="#roles">Choose account</a><a href="#about">How it works</a></nav><div className="header-actions"><Link to="/auth/job-seeker/signin">Sign in</Link></div></div></header>
    <main>
      ...
      <section id="roles" className="role-choice-grid">
        {roles.map((role) => <article key={role.key} className={`role-choice ${role.key}`}><div><Icon name={role.icon} size={28}/></div><h2>{role.title}</h2><p>{role.copy}</p><Link className="button primary" to={`/auth/${role.key}/signin`}>Continue as {role.title}</Link>{role.signup ? <Link className="role-create" to={`/auth/${role.key}/signup`}>Create {role.title} account</Link> : <small>Administrator accounts are created internally.</small>}</article>)}
      </section>
